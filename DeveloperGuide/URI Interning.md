...
In such scenarios it may be desirable to disable the interning behavior which may be done by setting the //InternUris// property of the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Options|Options]] class. With the property set to false even if the code calls the //Create()// method of the ##UriFactory## the URI will just be created rather than interned.

== Freeing Memory ==

Note that you can also periodically force the interned URIs to be cleared by calling the //Clear()// method of the ##UriFactory##. This will release the libraries references to the memory and this will then get cleaned up over time by the normal .Net GC process.