[[Home]] > [[User Guide]] > [[UserGuide/Utility Methods|Utility Methods]]

= Utility Methods =

The library provides a number of useful Utility Methods embodied in a single static [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Tools|Tools]] class which is part of the ##VDS.RDF## namespace. Anywhere you reference ##VDS.RDF## you'll be able to use the static methods described in this article.

== CopyNode ==

The //CopyNode()// method is designed to be use to copy nodes between graphs. As explained in the [[UserGuide/Library Overview|Library Overview]] when you create a triple all the nodes of that triple must come from the same graph. When you are working with multiple graphs and combining the information you may need to assert information from one graph into the other, the following example demonstrates this:
...
**Note:** Both of these methods have [[UserGuide/Extension Methods|Extension Methods]] defined which invoke them, in the above example the call to //Tools.CopyNode()// could be replaced with the following and still have the same meaning:
...
The //ResolveQName(String qname, INamespaceMapper nsmap, Uri baseUri)// function is used to resolve a prefixed name to a URI in the context of the given [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.INamespaceMapper|INamespaceMapper]] and Base URI. This is primarily used internally in some of the Parsers and in ##IUriNode## creation.
