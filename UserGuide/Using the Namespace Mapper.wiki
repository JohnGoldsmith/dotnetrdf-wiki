[[Home]] > [[User Guide]] > [[UserGuide/Using the Namespace Mapper|Using the Namespace Mapper]]
...
In order that prefixed names can be resolved into URIs various parts of the library must maintain a table of associations of Namespace Prefixes and their corresponding Namespace URIs. This function is provided by the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.INamespaceMapper|INamespaceMapper]] interface.

The most common place to encounter this interface is when working with graphs, any implementation of the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.IGraph|IGraph]] interface provides a property called //NamespaceMap// which returns the ##INamespaceMapper## associated with the graph.
