[[Home]] > [[User Guide]] > [[UserGuide/Working with Graphs|Working with Graphs]]

= Working with Graphs =

As previously introduced in the [[UserGuide/Library Overview|Library Overview]] the graph is the basic representation of a set of Triples used in the Library. All the graph classes are based on the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.IGraph|IGraph]] interface and mostly descend from the abstract base class [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.BaseGraph|BaseGraph]] - the most commonly used descendant being the basic [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Graph|Graph]] class.
...
The //BaseUri// property gets/sets the Base URI of the Graph. A Base URI if the URI against which any relative URIs are resolved as well as any prefixed names in the default namespace (if the default namespace is not explicitly defined). Graphs are not required to have a Base URI and by default this property returns null . Typically the Base URI gets set when you read RDF from a file and the RDF syntax defines a Base URI or you retrieve RDF from a URI using the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Parsing.UriLoader|UriLoader]] in which case the URI retrieved is the Base URI.
...
The //NamespaceMap// property returns the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.INamespaceMapper|INamespaceMapper]] instance which is associated with this Graph. The ##INamespaceMapper## is used to map prefixes to URIs in order to allow namespacing and prefixed name resolution, learn about using this class by reading [[UserGuide/Using the Namespace Mapper|Using the Namespace Mapper]].
...
The //Triples// property returns a [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.BaseTripleCollection|BaseTripleCollection]] object which is a collection of the Triples contained in the Graph and is probably the most important and most used property of a Graph. This allows you to enumerate over the Triples in the Graph in various ways but is most commonly used as previously seen for simple enumerating the Triples in the Graph and performing some action with each of them.

== Asserting and Retracting Triples ==

As already seen in the [[UserGuide/Library Overview|Library Overview]] and [[UserGuide/Hello World|Hello World]] one of the key functions of the Graph is to allow Triples to be asserted and retracted. For this the //Assert(…)// and //Retract(…)// methods are provided. Both of these methods can take either a single Tripleor an enumerable of Triples.
...
As also seen in [[UserGuide/Library Overview|Library Overview]] all Nodes must be created by an [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.INodeFactory|INodeFactory]] which all ##IGraph## implementations must also implement. Therefore a graph provides //CreateBlankNode(…)//, //CreateLiteralNode(…)// and //CreateURINode(…)//, see the earlier [[UserGuide/Library Overview|Library Overview]] for usage examples.
...
Graph Equality (aka Isomorphism) is supported through use of the standard //Equals(Object obj)// method. We also provide an additional overload //Equals(IGraph g, out Dictionary mapping)// which determines equality and if the Graphs are equivalent returns the mapping of Blank Nodes between the two graphs.  See [[UserGuide/Equality and Comparison|Equality and Comparison]] for more discussion of this.

== Graph Difference == 

If you know two Graphs are different it may be useful to know how they are different. The //Difference(IGraph g)// method determines the differences between two graphs returning a [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.GraphDiffReport|GraphDiffReport] which details the differences.

== Loading Graphs ==

The most common way of loading a Graph is to read RDF from a file/URI as described in [[UserGuide/Reading RDF|Reading RDF]] but there are other ways to read graphs, for example from persistent storage.

=== Triple Store Backed Graphs ===

Graphs can also be loaded from native Triple Stores which are accessible through the dotNetRDF API. To use a native Triple Store you'll need to use one of the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Storage.IStorageProvider|IStorageProvider]] implementations located in the ##VDS.RDF.Storage## namespace. Graphs can be loaded using an appropriate overload of the //LoadGraph()// method, see [[UserGuide/Triple Store Integration|Triple Store Integration]] for more details.

=== Using the StoreGraphPersistenceWrapper ===

Alternatively the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.StoreGraphPersistenceWrapper|StoreGraphPersistenceWrapper]] is a wrapper class that can be placed around any ##IGraph## instance and will persist any changes made when it is disposed of unless you discard them using it's //Discard()// method.
...
The most common way of saving a Graph is to save it to a file as described in [[UserGuide/Writing RDF|Writing RDF]] but you can also save it to other forms of persistent storage.

=== Triple Store Backed Graphs ===

The easiest way to save a Graph to a Triple Store is to use the //SaveGraph()// method of an ##IStorageProvider## implementation, see [[UserGuide/Triple Store Integration|Triple Store Integration]] for more details.
...
|= Class |= Description |
| [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Graph|Graph]] | In-memory graph with triple indexing |
| [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.NonIndexedGraph|NonIndexedGraph]] | In-memory graph without triple indexing |
| [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.StoreGraphPersistenceWrapper|StoreGraphPersistenceWrapper]] | A wrapper around another graph where changes are persisted to a backing store using an ##IStorageProvider##. |
| [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.ThreadSafeGraph|ThreadSafeGraph]] | A thread safe wrapper around another graph instance |

----

= Tutorial Navigation =

The next topic is [[UserGuide/Typed Values and Lists|Typed Values and Lists]], the previous topic was [[UserGuide/Writing RDF|Writing RDF]].
