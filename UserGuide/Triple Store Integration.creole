[[Home]] > [[User Guide]] > [[UserGuide/Storage API|Storage API]] > [[UserGuide/Triple Store Integration|Triple Store Integration]]

= 3rd Party Triple Store Integration =

While the API can represent Triple Stores as an in-memory collection of Graphs as discussed in Working with Triple Stores it also has the ability to integrate with a variety of 3rd party Triple Stores via our [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Storage.IStorageProvider|IStorageProvider]] and [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Storage.IAsyncStorageProvider|IAsyncStorage]] interfaces. Both these interfaces extend the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.IStorageCapabilities|IStorageCapabilities]] interface which provides information about what a specific implementation supports.
...
Take a look at the [[UserGuide/Storage/Providers|Storage Providers]] page for available implementations.
...
| //IOBehaviour// | Indicates detailed information about the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Storage.IOBehaviour|IOBehaviour]] that a store provides |
...
Some Stores provide SPARQL Query and/or Update support and this support is indicated by the ##IStorageProvider## implementation also implementing the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Storage.IQueryableStorage|IQueryableStorage]] and [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.IUpdateableStorage|IUpdateableStorage]] interfaces.

There are also [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Storage.IAsyncQueryableStorage|IAsyncQueryableStorage]] and [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Storage.IAsyncUpdateableStorage|IAsyncUpdateableStorage]] interfaces which are the async equivalents of the aforementioned interfaces.
...
The Async API essentially mirrors the synchronous API except that every method signature requires two additional parameters which are a [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Storage.ASyncStorageCallback|AsyncStorageCallback]] and Object which can be used to pass some state information to the callback.

The callback signature simply returns a reference to the ##IAsyncStorageProvider## that is invoking the callback, a set of [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Storage.AsyncStorageCallbackArgs|AsyncStorageCallbackArgs]] and the state information passed when the method was originally invoked.
