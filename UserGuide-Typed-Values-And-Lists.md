[[Home]] > [[User Guide]] > [[UserGuide/Typed Values and Lists|Typed Values and Lists]]
...
While this is relatively simple it's a pain to type out particularly if you have a lot of typed values you want to represent. To make the encoding of typed values into RDF the library offers a number of extension methods in the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.LiteralExtensions|LiteralExtensions]] class which provide //ToLiteral()// methods which you can apply on a variety of .Net types to get the equivalent RDF encoding of them e.g.
...
To decode values from RDF you can make use of the ##VDS.RDF.Nodes## namespace, while this is primarily used internally in our SPARQL engine it is also very useful to the end user. This namespace primarily provides a //AsValuedNode()// extension method which will give you back a [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Nodes.IValuedNode|IValuedNode]] instance. A ##IValuedNode## is simply a node that has a strongly typed value associated with it and provides a number of methods to retrieve the value of a node as a strongly typed .Net type e.g.
...
To ameliorate this problem we provide a number of methods in the [[http://www.dotnetrdf.org/api/index.asp?Topic=VDS.RDF.Extensions|Extensions]] class that allow you to manipulate RDF lists in a user friendly manner.
...
You can add and remove items from lists using convinience methods also. To add new items to the end of a list you can do the following:
...
The previous topic was [[UserGuide/Working with Graphs|Working with Graphs]], the next topic is [[UserGuide/Working with Triple Stores|Working with Triple Stores]]