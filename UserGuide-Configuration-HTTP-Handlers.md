[[Home]] > [[User Guide|UserGuide]] > [[Configuration API|UserGuide-Configuration-API]] > [[HTTP Handlers|UserGuide-Configuration-HTTP-Handlers]]

# Configuring HTTP Handlers 

HTTP Handler configuration is used to specify the settings for HTTP Handlers that you wish to use in ASP.Net applications. You can then either add handler registrations manually to your Web.config file or automatically using [[rdfWebDeploy|UserGuide-Tools-rdfWebDeploy]] in order to get the handlers running in your ASP.Net application (see [[Deploying with rdfWebDeploy|UserGuide-ASP-Deploying-With-rdfWebDeploy]] for a full example).
